...
class XMLToolFormatter(BaseToolFormatter):
    ...
    def format_for_prompt_injection(self, tools: List[Dict[str, Any]]) -> str:
        ...
        lines.append("Always emit reasoning first in this format:")
        lines.append("```")
        lines.append("<title>Short 4-12 word title</title>")
        lines.append("<thinking>Your reasoning chain here.</thinking>")
        lines.append("```")
        lines.append("")
