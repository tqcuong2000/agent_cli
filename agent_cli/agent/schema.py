...
_TOOL_PATTERN = re.compile(
    r"<tool>(.*?)</tool>", re.DOTALL
)
_ARGS_PATTERN = re.compile(
    r"<args>(.*?)</args>", re.DOTALL
)
...
class SchemaValidator(BaseSchemaValidator):
    ...
    def _extract_final_answer(self, text: str) -> Optional[str]:
        ...
        2. Clean text (with ``<thinking>`` removed) as implicit answer.
        """
        match = _FINAL_ANSWER_PATTERN.search(text)
        if match:
            return match.group(1).strip()

        # If no tags, the clean text (minus <thinking>) might be answer
    ...
    def _parse_json_args(
        ...
        if coerced is not None:
            logger.warning(
                "Coerced malformed JSON args for tool '%s'", tool_name
            )
            return coerced

        raise SchemaValidationError(
            f"Invalid JSON in <args> for tool '{tool_name}'. "
