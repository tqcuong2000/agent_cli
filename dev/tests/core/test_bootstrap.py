...
def test_create_app_returns_app_context():
    ...
    assert ctx.terminal_manager is not None
    assert ctx.system_info_provider is not None
...
def test_create_app_registers_terminal_tools():
    ctx = create_app()
    tool_names = set(ctx.tool_registry.get_all_names())
...
def test_create_app_registers_new_search_tools_and_removes_legacy_search():
    ctx = create_app()
    tool_names = set(ctx.tool_registry.get_all_names())

    assert "find_by_name" in tool_names
...
async def test_create_app_default_agent_prompt_includes_system_information():
    ...
    agent = ctx.agent_registry.get("default")
...
def test_builtin_agents_use_multi_action_defaults_from_data_registry():
    ...
    for agent_name in ("default", "coder", "researcher"):
        ...
        assert agent is not None
        assert agent.config.multi_action_enabled is True
...
@pytest.mark.asyncio
async def test_shutdown_calls_terminal_manager(monkeypatch: pytest.MonkeyPatch):
    ctx = create_app()
    await ctx.startup()
    ...
    monkeypatch.setattr(ctx.terminal_manager, "shutdown", _fake_shutdown)
