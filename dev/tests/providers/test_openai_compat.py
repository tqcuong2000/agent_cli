...
def get_mocked_compat(native_tools: bool = False, api_profile: dict | None = None):
...
@pytest.mark.asyncio
async def test_compat_generate_applies_web_search_api_profile_mutations():
    provider = get_mocked_compat(
        api_profile={
            "web_search": {
                "mutations": [
                    ...
                    {
                        "op": "merge_body",
                        "value": {"plugins": [{"id": "web"}], "metadata": {"ws": True}},
                    },
                ],
            }
        }
    ...
    call_args = provider.client.chat.completions.create.call_args[1]
    assert call_args["model"] == "llama-3-8b:online"
    assert call_args["extra_body"]["plugins"] == [{"id": "web"}]
    assert call_args["extra_body"]["metadata"] == {"ws": True}
