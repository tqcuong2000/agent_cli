...
def test_app_binds_command_parser_to_textual_app(tmp_path):
    ...
    assert isinstance(app.provider_overlay, ProviderOverlay)
...
@pytest.mark.asyncio
async def test_quit_action_shuts_down_context_before_exit(tmp_path):
    app = AgentCLIApp(root_folder=str(tmp_path))
    calls: list[str] = []
    ...
    app.app_context.shutdown = _shutdown  # type: ignore[method-assign]
    app.exit = lambda *args, **kwargs: calls.append("exit")  # type: ignore[method-assign]
