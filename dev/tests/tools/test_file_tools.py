...
async def test_read_file_tool(workspace, tmp_path):
    ...
    assert "  2: line2" in content_sliced
    assert "  4: line4" in content_sliced
    assert "  1: line1" not in content_sliced
    assert "  5: line5" not in content_sliced
...
@pytest.mark.asyncio
async def test_read_file_tool_can_disable_line_numbers(workspace, tmp_path):
    tool = ReadFileTool(workspace, show_line_numbers=False)
    file_path = tmp_path / "plain.txt"
    file_path.write_text("line1\nline2", encoding="utf-8")

    content = await tool.execute(path="plain.txt")
    assert "\nline1\nline2" in content
