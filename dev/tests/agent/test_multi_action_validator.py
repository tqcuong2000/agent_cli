...
def test_validate_empty_list_errors(registry: ToolRegistry) -> None:
    validator = MultiActionValidator(registry)
    with pytest.raises(SchemaValidationError, match="non-empty") as exc:
        validator.validate([])
    assert exc.value.error_id == "batch.empty_actions"


def test_validate_unknown_tool_errors(registry: ToolRegistry) -> None:
    validator = MultiActionValidator(registry)
    with pytest.raises(SchemaValidationError, match="Unknown tool") as exc:
        validator.validate([_action("missing_tool", 0)])
    assert exc.value.error_id == "batch.unknown_tool"
...
def test_validate_duplicate_action_ids_error(registry: ToolRegistry) -> None:
    ...
    with pytest.raises(SchemaValidationError, match="Duplicate action_id") as exc:
        validator.validate([a1, a2])
    assert exc.value.error_id == "batch.duplicate_action_id"
...
def test_validate_batch_size_limit_error(registry: ToolRegistry) -> None:
    ...
    with pytest.raises(SchemaValidationError, match="exceeds maximum") as exc:
        validator.validate([_action("read_file", 0), _action("read_file", 1)])
    assert exc.value.error_id == "batch.too_many_actions"
